...
if [ -f "$BEAMMP_DIR/bin/BeamMP-Launcher" ]; then
    ...
    echo "    - Vérification des fichiers dans bin/ :"
    ls -la "$BEAMMP_DIR/bin/"
    exit 1
fi
...
if [[ "$create_link" =~ ^[YyOo]$ ]]; then
    ...
    echo "    - Pour lancer le jeu : $BEAMMP_DIR/bin/BeamMP-Launcher"
fi

echo ""
echo "=== INSTALLATION TERMINÉE ==="
echo "Le launcher BeamMP est maintenant installé sur votre système Arch Linux !"
